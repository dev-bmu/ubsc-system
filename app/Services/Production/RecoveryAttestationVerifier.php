...
final class RecoveryAttestationVerifier
{
    ...
    public function hasValidActiveKeyConfiguration(): bool
    {
        ...
        $keyIds = $this->config->get('disaster_recovery.attestation.active_key_ids', []);
        if (! is_array($keyIds)
    ...
    {
        $keys = $this->config->get('disaster_recovery.attestation.verification_keys', []);
    ...
    public function isActiveKey(string $keyId): bool
    {
        return $this->hasValidActiveKeyConfiguration()
            && in_array(
                $keyId,
                (array) $this->config->get('disaster_recovery.attestation.active_key_ids', []),
                true,
            );
    }
    ...
    private function publicKey(string $keyId): \OpenSSLAsymmetricKey|false|null
    {
        ...
        $ecDetails = is_array($details) && is_array($details['ec'] ?? null)
            ? $details['ec']
            : [];
        $curve = strtolower((string) ($ecDetails['curve_name'] ?? ''));
    ...
    {
        if ($depth > 16) {
            throw new InvalidArgumentException('Recovery attestation nesting exceeds the safety limit.');
        }
        if (is_array($value)) {
            if (count($value) > 64) {
                throw new InvalidArgumentException('Recovery attestation collection exceeds the safety limit.');
            }
            ...
            foreach (array_keys($value) as $key) {
                if (! is_string($key)
                    ...
                    throw new InvalidArgumentException(
                        'Recovery attestation contains an invalid object key.',
                    );
                }
            }
        ...
        if (is_string($value)) {
            if (strlen($value) > 65_536) {
                throw new InvalidArgumentException('Recovery attestation string exceeds the safety limit.');
            }
        ...
        throw new InvalidArgumentException('Recovery attestation contains an unsupported value.');
    }
}
